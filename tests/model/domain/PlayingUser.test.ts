...
import { KillTargetResult } from '../../../src/model/domain/PlayingUser'
...
describe('killTarget', () => {
    test(`given a playing user, when he kills the target with the correct code, 
            his new target is the one of the killed user and kill count is increased`, async () => {
        ...
        expect(players[0].killCount).toBe(1)
    ...
    test(`given a playing user, when he tries to kill the target with the wrong code, 
        ...
        expect(players[0].killCount).toBe(0)
...
describe('startPlaying', () => {
    test(`given a non-playing user, when a new game is started, all fields are reset`, async () => {
        const players = createRingOfPlayers(2)
        players[0].killTarget(generateKillCodeFromSeed(1))
        
        await setTimeout(2)
        const startDate = new Date()
        players[0].startPlaying(startDate)

        expect(players[0].killCount).toBe(0)
        expect(players[0].lastKill).toEqual(startDate)
    })
})
